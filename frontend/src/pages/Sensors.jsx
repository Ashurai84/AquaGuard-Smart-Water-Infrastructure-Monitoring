...
const Sensors = () => {
  ...
  useEffect(() => {
    fetchSensors();
    const interval = setInterval(fetchSensors, 6000);
              ...
              <tr key={s.id}>
                <td style={{ fontWeight: '600' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <Radio size={16} className="text-secondary" style={{ color: s.status === 'Active' ? 'var(--accent-cyan)' : 'var(--text-muted)' }} />
                    <span>{s.name}</span>
                  </div>
                </td>
                <td>
                ...
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                    <span className={`pulse-indicator ${s.status === 'Active' ? 'green' : 'red'}`} />
                    <span style={{ fontSize: '0.9rem', color: s.status === 'Active' ? 'var(--accent-green)' : 'var(--accent-red)' }}>
                      {s.status}
                    </span>
                  </div>
                </td>
                <td style={{ fontFamily: 'var(--font-mono)', fontWeight: '700', fontSize: '1rem', color: s.status === 'Active' ? 'var(--text-primary)' : 'var(--text-muted)' }}>
                  {s.status === 'Active' ? `${s.current_value.toFixed(1)} ${s.unit}` : 'N/A'}
                </td>
